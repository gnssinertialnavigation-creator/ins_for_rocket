...
//---------------------------------------------------------
void get_ins_04_pos( double p_ecef[3] )
{
  memcpy( &p_ecef[0], &pos_ecef[0], sizeof( double ) * 3 );
}

//---------------------------------------------------------
void get_ins_04_vel( double v_ecef[3] )
{
  memcpy( &v_ecef[0], &vel_ecef[0], sizeof( double ) * 3 );
}

//---------------------------------------------------------
void get_ins_04_qu_b2e( double q[4] )
{
  memcpy( &q[0], &qu_b2e[0], sizeof( double ) * 4 );
}

//---------------------------------------------------------
void get_ins_04_bias_force( double bf[3] )
{
  memcpy( &bf[0], &bias_force[0], sizeof( double ) * 3 );
}

//---------------------------------------------------------
void get_ins_04_bias_gyro( double bg[3] )
{
  memcpy( &bg[0], &bias_gyro[0], sizeof( double ) * 3 );
}

//---------------------------------------------------------
void reset_ins_04( const double p_ecef[3], const double v_ecef[3], const double qu[4] )
{
  set_ins_pos( p_ecef );
  set_ins_vel( v_ecef );
  set_ins_qu_b2e( qu );
...
//---------------------------------------------------------
void ins_04( const double time_last, const double p_ecef[3], const double v_ecef[3],
             ...
             const double f_body[3], const double gyro_body[3], const double dt )
{
  ...
  if( kalman_update_02( time_last, measured_attitude,
                        ...
                        meas, f_b, gy_b, imu_force_noise_std_04,
                        imu_gyro_noise_std_04, imu_force_bias_std_04,
                        imu_gyro_bias_std_04 ) )
  {
    set_ins_pos( &pos[0] );
    set_ins_vel( &vel[0] );
    set_ins_qu_b2e( &qu[0] );
...
static void reset_ins_pk( void )
{
  ...
  for( idx=0; idx<3; idx++ )
  {
    Pk[idx][idx] = 10000;
    Pk[idx+3][idx+3] = 4;
    Pk[idx+10][idx+10] = pow( ( 10 * imu_force_bias_std_04 ), 2 );
    Pk[idx+13][idx+13] = pow( ( 10 * imu_gyro_bias_std_04 ), 2 );
  }
  for( idx=0; idx<4; idx++ )
                            ...
                            const double time_last, const double dt )
{
  ...
  get_ins_04_pos( &st0[0] );
  get_ins_04_vel( &st0[3] );
  get_ins_04_qu_b2e( &st0[6] );
  ...
  pk_propagator_02( Pk, imu_force_noise_std_04, imu_gyro_noise_std_04,
                    imu_force_bias_std_04, imu_gyro_bias_std_04, gy_b,
                    f_b, &st0[0], &st0[6], dt );
}
