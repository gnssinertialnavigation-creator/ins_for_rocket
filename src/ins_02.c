             ...
             const double f_body[3], const double gyro_body[3], const double dt )
{
  ...
  if( kalman_update( time_last, measured_attitude,
                     ...
                     meas, f_ecef, gy_ecef, imu_force_noise_std_02,
                     imu_gyro_noise_std_02, imu_force_bias_std_02,
                     imu_gyro_bias_std_02 ) )
  {
    set_ins_pos( &pos[0] );
    set_ins_vel( &vel[0] );
    set_ins_qu_b2e( &qu[0] );
...
static void reset_ins_pk( void )
{
  ...
  for( idx=0; idx<3; idx++ )
  {
    ...
    Pk[idx+9][idx+9] = pow( ( 10 * imu_force_bias_std_02 ), 2 );
    Pk[idx+12][idx+12] = pow( ( 10 * imu_gyro_bias_std_02 ), 2 );
  }
}
                            ...
                            const double time_last, const double dt )
{
  ...
  get_ins_02_pos( &st0[0] );
  get_ins_02_vel( &st0[3] );
  get_ins_02_qu_b2e( &st0[6] );
  ...
  pk_propagator( Pk, imu_force_noise_std_02, imu_gyro_noise_std_02,
                 imu_force_bias_std_02, imu_gyro_bias_std_02, gy_b,
                 f_b, &st0[0], &st0[6], dt );
}
